...
function EditOutlay({ show, onHide, searchDate, gastoAEditar }) {
  ...
  useEffect(() => {
    if (gastoAEditar && gastoAEditar.id) {
      ...
      const date = gastoAEditar.createdAt
        ? new Date(gastoAEditar.createdAt).toISOString().split("T")[0]
        : "";
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    const result = await dispatch(updateOutlay({ id: gastoAEditar.id, ...data }, searchDate));
    setLoading(false);
      ...
      <ModalContent className="bg-white">
        <form onSubmit={handleSubmit(onSubmit)}>
          <ModalHeader className="flex flex-col gap-1">
            ...
            <span className="text-sm font-medium text-slate-500">Actualiza la información del gasto seleccionada</span>
          </ModalHeader>
